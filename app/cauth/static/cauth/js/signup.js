...
function integrate_validate_email() {
    ...
    email_input.addEventListener('input', async function () {
        ...
        console.log("integrate_validate_email res", res, res.data, res.status);
        if (res.status === 200) {
            let svg = 'middle.svg'
            ...
            email_svg.src = `/static/cauth/images/icons/${svg}`;
        }
    });
}
